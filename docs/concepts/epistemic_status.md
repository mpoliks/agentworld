...
Every knob in `TopologyConfig`, `PopulationConfig`, `StackConfig`, `DragConfig`, `DifferentialConfig`, `LastMileConfig`, `ImperialConfig`, and `RegionConfig` is one of three kinds:
...
| `DragConfig.coasean_dampener`, `adaptive_*` | drag | Speculative | Models the "agent-as-palliative" hypothesis; cannot be fitted because the mechanism does not yet exist at scale. |
