...
def test_legacy_matches_pre_split_layout() -> None:
    """Under legacy, the eight keys in `world.rngs` all point at the
    *same* `Generator` instance. A change to the build code that
    accidentally split them would silently shift every canonical
    baseline; this test catches it before the regression suite does.
    """
