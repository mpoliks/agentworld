...
def capability_update(pop: Any, wealth_delta: np.ndarray, cfg: Any) -> None:
    ...
    pop.capability += cfg.capability_learning_rate * normalized_reward.astype(np.float32)
    pop.capability -= np.float32(cfg.capability_decay_rate)
    np.clip(pop.capability, 0.01, 0.99, out=pop.capability)


def wealth_depreciation(pop: Any, cfg: Any) -> None:
    """Apply per-step wealth maintenance depreciation."""
    pop.wealth *= np.float32(1.0 - cfg.wealth_depreciation)
