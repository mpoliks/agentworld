...
.callout { font-family: var(--serif); font-size: 16px; line-height: 1.55; padding: 18px 22px; background: var(--panel); margin: 24px 0; color: var(--text); }
...
$$</div>
      ...
      <p>One interesting note on simulation method before we dig into a few conclusions. Twenty-one of the twenty-five scenarios on this dashboard run on a pretty complex simulation stack — sector-block trading network calibrated to Atalay et al. 2011, t-copula correlated noise calibrated to the BEA 2022 input-output matrix, and a self-exciting Hawkes folding kernel calibrated to the Bacry &amp; Muzy 2015 endogeneity ratio. Boom. The four exceptions to that rule are the productive-folding scenarios (<i>Productive Cathedral</i>, <i>Productive Baroque</i>, <i>Derivatives Revolution</i>) and one adversarial-search variant (<i>Baroque (High Welfare)</i>) whose hand-tuned parameters don't make sense in that context. This means that when you compare <i>Slop Market</i> to <i>Productive Cathedral</i> across the high-$\alpha$ band, the only meaningful difference is productive folding. When you compare <i>Coasean Paradise</i> to <i>Baroque Cathedral</i>, both are using the same engine. Welfare numbers on this dashboard are systematically higher than they would be on a well-mixed default — by roughly 60% across the board — because realistic trading networks pre-match compatible pairs, raising matching efficiency. But it's super important to note that this simulation engine pretty much just changes the welfare height the basins can support, but not which basin the scenario is in.</p>

      <p>The six conclusions the simulator keeps surfacing, across scenarios and seeds, are enumerated in the callout in §2. In short: <em>(i)</em> direct-trade regimes (e.g. Coarsen bargaining) are unstable on their own — staying there is an active engineering task, not a passive default; <em>(ii)</em> productive sub-markets raise welfare in absolute terms but do not move the share of measured activity that reaches humans, so $\mathrm{EBI}$ and welfare can both come out high in the same regime; <em>(iii)</em> a targeted tax on agent-to-agent transaction volume is the one corrective lever the simulator finds, and it is bounded by the share of nominal activity that would have produced welfare if not deterred; <em>(iv)</em> wealth inequality moves only when bargaining power is equalized before trades happen, not by post-trade redistribution; <em>(v)</em> the share of the economy that any human is party to collapses with $\alpha$ independently of welfare, raising a separate concern about democratic legibility; and <em>(vi)</em> even when agents are free to learn their own preferred $\alpha$, they settle at upper-mid $\alpha$ rather than at either extreme. The atlas in §2 makes (i) and (ii) visible at a glance. The detail panes in §4, the §5 Sankey, and the §6 overlays make the rest legible.</p>
