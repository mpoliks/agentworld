...
SCENARIO_DESCRIPTIONS = {
    ...
    "coasean_paradise_networked": "coasean_paradise run on a Barabási-Albert scale-free network with the Atalay et al. 2011 degree exponent, with t-copula coupled noise (df = 4) calibrated to BEA 2022 input-output correlations and a Hawkes self-exciting folding kernel calibrated to the Bacry & Muzy 2015 endogeneity ratio. Tests whether the smooth attractor survives a topology in which a few hub traders dominate the connectivity and shocks propagate along high-traffic edges.",
    "baroque_cathedral_networked": "baroque_cathedral on a sector-block stochastic block model with the same t-copula noise and Hawkes folding kernel. Tests whether the fractal attractor is robust to clustering, or whether community structure makes the cathedral fragile to the correlated shocks the smooth attractor would absorb.",
    "synthetic_consumers_v2": "synthetic_consumers with demand-side weighting on. Trades that ultimately reach a human consumer count for full welfare; pure agent-to-agent trades are credited at the 15% floor that accounts for indirect downstream benefits. Reports the gap between what the economy printed and what humans got, separating accounting from welfare in a way the unweighted version cannot.",
    ...
    "pigouvian_light": "A 10% tax on agent-to-agent trades, recycled to human wealth. The same configuration as synthetic_consumers_v2 with a corrective transfer added on top. Tests whether a small Pigouvian wedge recovers most of the welfare lost to demand-side substitution, or whether a steeper rate is needed.",
    "pigouvian_heavy": "A 35% agent-to-agent tax, recycled to human wealth. Tests overcorrection: whether the tax can be set high enough that human welfare overshoots the no-tax baseline. The point at which the tax stops being corrective and starts being extractive is one of the cleaner mechanism-design questions the simulator can answer.",
    "pigouvian_friction": "Agent-to-agent tax recycled as a friction subsidy on human-to-agent trades rather than a wealth transfer. Tests whether bringing humans back into the executable trade set via cost relief — the supply side of welfare — is more efficient than recycling the tax to the demand side as cash.",
    "pigouvian_baroque": "The agent-to-agent tax applied on top of a productive-fractal baseline. Tests whether the tax correctly distinguishes productive sub-trades from purely extractive ones, or whether it suppresses both indiscriminately. A test of whether the corrective transfer is a precision instrument or a blunt one.",
